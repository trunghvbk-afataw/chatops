...
- Telegram poller now always advances the saved update offset after inspecting an update, even if command handling fails.
- This prevents replaying the same Telegram command repeatedly when a handler raises or response sending fails mid-cycle.

## [0.1.3] - 2026-04-16

### Fixed

- Fixed `telegram_poller` response sending call to `send_telegram_message` by passing arguments in the correct order: `(chat_id, token, ...)`.
- This resolves Telegram `404 Not Found` errors when the poller handles a command and sends a reply.
...
[0.1.4]: https://github.com/trunghvbk-afataw/chatops/releases/tag/v0.1.4
